...
class DataProcessor:
    ...
    def normalize_data(self) -> pd.DataFrame:
        ...
        combined_df = combined_df[['post_id', 'comment_id', 'title', 'text', 'created_time', 'subreddit', 'url']]
...
class SentimentAnalysisPipeline:
    ...
    def __init__(self, data_path: str, model_path: str, tokenizer_path: str):
    ...
    def sentiment_distribution(self):
        ...
        cleaned_df = self.processor.df
        cleaned_df['predicted_sentiment'] = self.analyzer.predict_batch(cleaned_df['text'].tolist())
        cleaned_df['created_time'] = pd.to_datetime(cleaned_df['created_time'])
        cleaned_df['polarity'] = cleaned_df['text'].apply(lambda x: TextBlob(x).sentiment.polarity) # Calculate TextBlob polarity score
        ...
        sentiment_counts_formatted['Percentage'] = sentiment_percentages_formatted.map("{:.2f}%".format)
        ...
        print(f"Overall Sentiment: {overall_sentiment} {self.emoji_map[overall_sentiment]}")
        print("=" * 50 + "\n")
        ...
        with open('./output/sentiment_distribution.txt', 'w') as f:
            ...
            f.write("\n\nOverall Sentiment: ")
            f.write(f"{overall_sentiment} {self.emoji_map[overall_sentiment]}")
        
        print("Sentiment distribution saved to './output/sentiment_distribution.txt'.\n")
    
    def sentiment_intensity(self):
        """Calculates sentiment intensity using VADER."""
        print("Calculating sentiment intensity...\n")
        cleaned_df = self.processor.df
        analyzer = SentimentIntensityAnalyzer()

        # Calculate sentiment intensity using VADER
        cleaned_df['sentiment_intensity'] = cleaned_df['text'].apply(lambda x: analyzer.polarity_scores(x)['compound'])

        # Get sentiment instensity distribution
        intensity_counts = cleaned_df['sentiment_intensity'].value_counts()

        # Calculate percentages
        total_intensity = intensity_counts.sum()
        intensity_percentages = (intensity_counts / total_intensity) * 100
        ...
        intensity_counts_formatted.columns = ['Intensity', 'Count']
        ...
        print(f"Overall Sentiment Intensity: {overall_intensity} {self.emoji_map[overall_intensity]}")
        print("=" * 50 + "\n")
        ...
        with open('./output/sentiment_intensity.txt', 'w') as f:
            ...
            f.write(f"{overall_intensity} {self.emoji_map[overall_intensity]}")
    ...
    def generate_wordclouds(self):
        ...
        def extract_nouns_adjectives(text: str) -> str:
            blob = TextBlob(text)
            nouns_adjectives = [word for word, tag in blob.tags if tag in ('NN', 'NNS', 'NNP', 'NNPS', 'JJ', 'JJR', 'JJS')]
            return ' '.join(nouns_adjectives)

        cleaned_df['nouns_adjectives'] = cleaned_df['text'].apply(extract_nouns_adjectives)
        ...
        def show_wordcloud(sentiment: str):
            ...
            plt.title(f"{sentiment.capitalize()} Sentiment Word Cloud", pad=20, fontsize=16)
            plt.axis('off')

            # Save the word cloud
            plt.savefig(f'./output/{sentiment}_wordcloud.png')
            print(f"Word cloud for {sentiment} sentiment saved to './output/{sentiment}_wordcloud.png'.\n")
    ...
    def sentiment_trend_analysis(self):
        ...
        polarity_trend = cleaned_df.groupby(['date', 'predicted_sentiment']).size().unstack().fillna(0)
        ...
        print("Sentiment trend analysis over time saved to './output/sentiment_trend.png'.\n")
...
# Keywords in Sentiments: Get top 5 words for each sentiment

# Aspect-Based Sentiment Analysis (ABSA): Determine what specific aspects of the show people are commenting on and their sentiment toward those aspects

# Analyze polarity trends over time

# Extract named entities (e.g. Beth, Benny, Jolene) and analyze sentiment toward specific characters

# Topic Modeling and Sentiment: Determine the topics people are discussing and the sentiment toward those topics (e.g. LDA or BERTopic)